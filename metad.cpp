...
C++ code to simulate metadynamics (MetaD) and obtain a free energy surface (FES) from a potential energy surface (PES).
*/
...
class MD_Simn
{
                                   ...
                                   Pes_membfunc pes_func, double h = 0.001)
    {
        ...
        for (auto x_i : xt)
        {
            vector<double> xt_fwd = xt;
            vector<double> xt_bwd = xt;
                           ...
                           vector<double> gauss_w, double dep_stride, vector<double> x0, Pot_funcs *pot_funcs_obj, Pes_membfunc pes_func)
{
    ...
    T_hi = T2;
    W = gauss_h;
    sigma = gauss_w, tau_G = dep_stride;
    omega = W / tau_G;
    n_gauss = n_steps / n_dep;
    xt = x0;
    wt = false;
    pes_inst = pot_funcs_obj;
    pes = pes_func;
    if (wt)
    {
        vector<double> W_walkers(W, N_replicas);
    }
